...
Тест создаёт изолированный template и Hierarchy Setting, переводит Item из 5 в 0 и проверяет ровно один email invocation.
...
После source deployment остаются org-specific действия:

1. записать Unsplash Access Key;
2. заполнить Inventory Notification Settings;
3. назначить permission sets и manager flag;
4. создать Admin user с email `dev@truesolv.com`;
5. запустить server-side Apex tests;
6. создать и загрузить unmanaged package.

GitHub publication и отправка repository URL по email относятся к delivery, а не к metadata deployment. Локальный Prettier parser проверяет грамматику, но только Salesforce server может подтвердить типы, Flow runtime и org-specific permissions.

## 12. Как самостоятельно проверить сценарий

### Каталог

1. Создайте 3–4 Item разных Type и Family.
