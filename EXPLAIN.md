...
### Зачем Item хранит автора фотографии

`Image__c` содержит hotlinked URL из `photo.urls.small`. Этого недостаточно для корректного использования Unsplash API: рядом с фотографией должны быть имя автора, ссылка на его профиль и ссылка на Unsplash.

Поэтому Item дополнен двумя provenance-полями:

- `Unsplash_Photographer__c` — имя из `photo.user.name`;
- `Unsplash_Profile_URL__c` — проверенный `photo.user.links.html` с `utm_source=item_purchase_tool&utm_medium=referral`.
...
1. авторизация;
2. валидация полей;
3. search callout по нормализованному Item Name;
4. проверка image URL, автора, profile URL и `download_location`;
5. tracking callout по возвращённому `download_location`;
6. user-mode insert Item с изображением и provenance-полями.

Salesforce запрещает обычный callout после незавершённого DML в той же транзакции. Оба вызова Unsplash выполняются до insert, поэтому нет ошибки `uncommitted work pending`. Если tracking не подтверждён, Item не создаётся.

`UnsplashClient.Photo` — внутренний DTO, который переносит четыре связанных значения как одно целое: image URL, имя автора, attribution URL и tracking path. Это не даёт случайно сохранить изображение без автора или вызвать другой endpoint.

Даже HTTP 200 не считается доверенным автоматически. Клиент принимает image только с `images.unsplash.com`, профиль только с `unsplash.com`, а tracking URL — только с точным host `api.unsplash.com` и path `/photos/{id}/download`. Все безопасные query-параметры из `download_location`, включая `ixid`, передаются без изменения; fragment, CR/LF и неподдерживаемые символы не проходят validation.
...
- `itemImage` — единое изображение, placeholder и видимая Unsplash attribution;
...
Общие shell, image, presentation и event modules убирают повторение разметки и логики между карточкой и модальными окнами, сохраняя у каждого компонента одну понятную ответственность. Tile, Details и Cart передают авторские данные одному `itemImage`, поэтому текст и безопасные links не расходятся между экранами.
...
Tracking использует тот же Named Credential и относительный path, полученный после строгой проверки `photo.links.download_location`:

`callout:Unsplash_API/photos/{id}/download?...`
...
- явный FLS для permissionable optional fields: Account Number, Industry, Item Description/Image/attribution, Line Total и manager flag;
- чтение обязательных и Master-Detail fields через object access: API 67.0 не допускает отдельные `fieldPermissions` для таких полей;
...
- create/edit Item и edit FLS для optional Item fields;
...
Factory создаёт общие Account, Item, Purchase, lines, users и cart DTO. Он убирает копирование setup-кода и оставляет тестам только важное различие сценариев. Прикладные test users получают `Minimum Access - Salesforce` с fallback на `Standard User`, а не профиль запускающего администратора; поэтому admin CRUD/FLS не маскирует проверяемые permission sets.
...
`HttpCalloutMock` заменяет сеть. Тесты проверяют exact search и tracking endpoints, сохранение query-параметров, `Accept-Version`, success, 429, 5xx, invalid JSON/shape, пустой result, неподдерживаемые hosts/fragment и отсутствие insert при ошибке tracking.
...
В Dev Org `item-purchase-dev` пройден полный путь:
...
Проверьте principal status, manager permission set, UserExternalCredential access, Allow Formulas in HTTP Header и лимит Unsplash. Ошибка может относиться как к search, так и к обязательному tracking request. Если вместо фотографии показан placeholder, у записи нет полной пары `Unsplash_Photographer__c` + доверенный `Unsplash_Profile_URL__c`.
...
| Hotlinking               | Прямое использование image URL, который вернул Unsplash API          |
| Attribution              | Видимое указание фотографа и Unsplash с referral links               |
